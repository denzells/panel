...
local function loadModule(path)
    local content
    local ok = pcall(function() content = game:HttpGet(RAW_BASE .. path) end)
    ...
    if not fn then
        warn("[PanelBase] Error compilando " .. path .. ": " .. tostring(err))
        return nil
    end
...
local BADGE = isAdmin and {
    bg = Color3.fromRGB(28,28,28), stroke = Color3.fromRGB(180,180,180), stAlpha = 0.4,
    text = "⭐  Admin", col = Color3.fromRGB(235,235,235),
} or {
    bg = Color3.fromRGB(16,42,16), stroke = Color3.fromRGB(40,180,70), stAlpha = 0.3,
    text = "✓  Verified", col = Color3.fromRGB(60,210,90),
}
...
local verifiedBadge = mk("Frame", {
    Size = UDim2.new(0, 84, 0, 22), Position = UDim2.new(0, 129, 0.5, -11),
...
mk("TextLabel", {
    Text = BADGE.text, Font = Enum.Font.GothamBold, TextSize = 9,
    TextColor3 = BADGE.col, BackgroundTransparency = 1,
...
mk("Frame", {
    ...
    BackgroundColor3 = C.LINE, BorderSizePixel = 0, ZIndex = 7,
...
for i, td in ipairs(TDEFS) do
    ...
    hit.MouseButton1Click:Connect(function()
        ...
        actNav = i; tPages[i].Parent.Visible = true; pill.ZIndex = 15
...
Settings.build(tPages[4], {
    ...
    title1 = title1, title2 = title2, title3 = nil,
    tw = tw, mk = mk, rnd = rnd,
    Win = Win, NavBar = NavBar, anim = anim,
    isAdmin = isAdmin, savedExpiry = savedExpiry,
})
